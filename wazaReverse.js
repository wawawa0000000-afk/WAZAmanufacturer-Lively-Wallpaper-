...
document.querySelector('.kanji').style.color = '#fff';
...
float renderScene(vec2 uv, float t) {
    ...
    // 【画質改善の鍵】1ピクセルあたりの距離を計算し、ボケ幅を最小限（1ピクセル分）にする
    // 0.01 などの固定値ではなく、解像度に依存しないシャープさを出します
    float edgeSmoothing = 0.7 / resolution.y; 
    ...
    for(int i = 1; i <= circleCount; i++) {
        ...
        // 【修正ポイント】固定の 0.01 を edgeSmoothing に変更
        float alpha = smoothstep(edgeSmoothing, 0.0, d) * mask;
        
        finalAlpha = max(finalAlpha, alpha);
    }
    // waza.js の場合は 1.0 - finalAlpha / wazaReverse.js の場合は finalAlpha
    return finalAlpha; 
}
...
>>>>>>> a8dc39b4aeca3c66b27d16ed27b34fdfd7482f9c
requestAnimationFrame(render);